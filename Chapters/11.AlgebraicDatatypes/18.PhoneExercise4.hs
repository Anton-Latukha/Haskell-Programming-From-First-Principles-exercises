...
mostUsedSymbol :: String -> Symbol
mostUsedSymbol text = snd (maximum records)
  where
    records :: [(Int, Symbol)]
    records = fmap (\ cluster -> (length cluster, head cluster)) (clusterization (sort (fmap toLower text)))
